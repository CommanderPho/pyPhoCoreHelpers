...
# and `pip install pyPhoCoreHelpers` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "pyPhoCoreHelpers", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n pyPhoCoreHelpers -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "pyPhoCoreHelpers", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "pyPhoCoreHelpers", "YOUR_CONSOLE_SCRIPT"]
