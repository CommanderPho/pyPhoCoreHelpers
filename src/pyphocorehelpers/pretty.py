...
def pretty(obj, verbose=False, max_width=MAX_LINE_LENGTH, newline='\n', max_seq_length=MAX_SEQ_LENGTH):
    """
...
def pprint(obj, verbose=False, max_width=MAX_LINE_LENGTH, newline='\n', max_seq_length=MAX_SEQ_LENGTH):
    """
...
# from builtins.pprint _______________________________________________________________________________________________ #
...
# end from builtins.pprint _______________________________________________________________________________________________ #
...
class PrettyPrinter(_PrettyPrinterBase):
    ...
    def __init__(self, output, max_width=MAX_LINE_LENGTH, newline='\n', max_seq_length=MAX_SEQ_LENGTH, compact=True):
        self.output = output
...
class RepresentationPrinter(PrettyPrinter):
    ...
    def __init__(self, output, verbose=False, max_width=MAX_LINE_LENGTH, newline='\n',
        singleton_pprinters=None, type_pprinters=None, deferred_pprinters=None,
        max_seq_length=MAX_SEQ_LENGTH, compact=True):

        PrettyPrinter.__init__(self, output, max_width, newline, max_seq_length=max_seq_length, compact=compact)
...
def wide_pprint(*args, **kwargs):
    ...
    """
    kwargs['width'] = MAX_LINE_LENGTH
    kwargs['compact'] = True
    return sys_pprint.pprint(*args, **kwargs)

## Override IPython pprint:
def wide_pprint_ipython(obj, **kwargs):
    import builtins
    kwargs['width'] = MAX_LINE_LENGTH
    kwargs['compact'] = True
    return builtins.print(ipython_pretty(obj, **kwargs))

def wide_pprint_jupyter(obj, p, cycle, **kwargs):
    if cycle:
        p.text("Circular reference detected")
    else:
        # sys_pprint.pprint(obj, stream=p, compact=True, width=_DESIRED_LINE_WIDTH)  # Update here.
        # output = pretty(obj, compact=True, width=_DESIRED_LINE_WIDTH)
        output = sys_pprint.pformat(obj, compact=True, width=MAX_LINE_LENGTH)
		...
		# printer = pprint.PrettyPrinter(stream=stream, indent=indent, width=width, depth=depth, compact=compact, sort_dicts=sort_dicts)
		# printer.pprint(obj)

		# # printer = RepresentationPrinter(sys.stdout, False, MAX_LINE_LENGTH, '\n', max_seq_length=max_seq_length)
		# # printer.pretty(obj)
		# # printer.flush()
		# sys.stdout.write(newline)
		# sys.stdout.flush()

		# # pprint.pprint(obj, stream=p, compact=True, width=MAX_LINE_LENGTH)  # working ish
		# # output = pretty(obj, compact=True, width=_DESIRED_LINE_WIDTH)
		# # output = pprint.pformat(obj, compact=True, width=_DESIRED_LINE_WIDTH)
		# # for line in output.splitlines():
		# #     p.text(line)
...
# get_ipython().display_formatter.formatters['text/plain'].for_type(object, wide_pprint_jupyter)
