...
def is_consecutive_no_gaps(arr, enable_debug_print=...):
...
def build_pairwise_indicies(target_indicies, debug_print=...):
...
def get_dict_subset(a_dict, included_keys=..., require_all_keys=...):
...
def validate_reverse_index_map(value_to_original_index_reverse_map, neuron_IDXs, cell_ids, debug_print=...):
    """
...
def flatpaths_to_nested_dicts(flat_paths_form_dict, default_value_override=..., flat_path_delimiter=..., debug_print=...):
...
_GLOBAL_MAX_DEPTH = ...
def nested_dicts_to_flatpaths(curr_key, curr_value, max_depth=..., depth=..., flat_path_delimiter=..., debug_print=...):
...
class BinningContainer:
    ...
    def setup_from_edges(self, edges: np.ndarray, edge_info: Optional[BinningInfo] = ...):
        ...
    


def compute_spanning_bins(variable_values, num_bins: int = ..., bin_size: float = ..., variable_start_value: float = ..., variable_end_value: float = ...):
    """[summary]
...
def debug_print_1D_bin_infos(bins, label=...):
...
def compute_paginated_grid_config(num_required_subplots, max_num_columns, max_subplots_per_page=..., data_indicies=..., last_figure_subplots_same_layout=..., debug_print=...):
...
def build_spanning_grid_matrix(x_values, y_values, debug_print=...):
...
class Paginator:
    ...
    @classmethod
    def init_from_data(cls, sequencesToShow, max_num_columns=..., max_subplots_per_page=..., data_indicies=..., last_figure_subplots_same_layout=...):
