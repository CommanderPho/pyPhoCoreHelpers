...
This type stub file was generated by pyright.
"""
def truncating_list_repr(items, max_full_display_n_items: int = ..., truncated_num_edge_items: int = ...):
...
class ANSI_Coloring:
    ...
    @classmethod
    def ansi_highlight_only_suffix(cls, type_string, suffix_color=...):
...
class DocumentationFilePrinter:
    ...
    def save_documentation(self, *args, skip_save_to_file=..., skip_print=..., custom_plain_text_formatter=..., custom_rich_text_formatter=..., **kwargs):
        """
...
def generate_html_string(input_str, color=..., font_size=..., bold=..., italic=...):
...
def format_seconds_human_readable(seconds, h_m_s_format_array=..., fixed_width=...):
...
def print_seconds_human_readable(seconds, h_m_s_format_array=..., fixed_width=...):
...
def print_filesystem_file_size(file_path, enable_print=...):
...
_GLOBAL_MAX_DEPTH = ...
def print_keys_if_possible(curr_key, curr_value, max_depth=..., depth=..., omit_curr_item_print=..., additional_excluded_item_classes=..., non_expanded_item_keys=..., custom_item_formatter=...):
...
def print_value_overview_only(a_value, should_return_string=...):
...
def min_mean_max_sum(M, print_result=...):
...
def document_active_variables(params, include_explicit_values=..., enable_print=...):
...
def build_module_logger(module_name=..., file_logging_dir=..., debug_print=...):
